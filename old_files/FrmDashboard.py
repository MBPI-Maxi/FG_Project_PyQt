...
from PyQt6.QtWidgets import (
    ...
    QDateEdit, QTextEdit, QFormLayout, QMessageBox, QCheckBox,
    QGraphicsDropShadowEffect, QTabWidget
)
from PyQt6.QtGui import QFont, QIcon, QPixmap, QValidator, QColor
from PyQt6.QtCore import (
    Qt, QSize, QPropertyAnimation, QEasingCurve, QEvent, QTimer, QDate
)
...
class SimpleIncomingForm(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(["Seq", "Control No", "Date", "Material Code", "Qty", "Notes"])
        ...
        self.btn_delete = QPushButton(fa.icon('fa5s.trash-alt', color='white'), " Delete");
    ...
    def load_records(self):
        ...
        query = sql.SQL(
            "SELECT {f_seq}, {f_ctrl}, {f_date}, {f_mat}, {f_qty}, {f_note} FROM {tbl} WHERE ({f_del} = %s OR %s = TRUE)").format(
            f_seq=sql.Identifier(COL_SEQ), f_ctrl=sql.Identifier(COL_CTRLNUM), f_date=sql.Identifier(COL_DATE),
            f_mat=sql.Identifier(COL_MATCODE), f_qty=sql.Identifier(COL_QTY), f_note=sql.Identifier(COL_NOTE),
            tbl=sql.Identifier(TBL_INCOMING), f_del=sql.Identifier(COL_DELETED))
            ...
            with self.conn.cursor() as cursor:
                ...
                for row_data in records:
                    ...
                    for i, data in enumerate(row_data):
                        item_text = str(data.strftime('%Y-%m-%d')) if isinstance(data, datetime.date) else str(data)
    ...
    def load_selected_record_for_edit(self):
        ...
        self.date_entry.setDate(QDate.fromString(self.table.item(selected_row, 2).text(), 'yyyy-MM-dd'))
    ...
    def save_record(self):
        if not all([self.txt_ctrlnum.text().strip(), self.txt_matcode.text().strip()]):
            QMessageBox.warning(self, "Validation Error", "Control No and Material Code are required.")
            return
        try:
            ...
            return
        data = {'t_ctrlnum': self.txt_ctrlnum.text().strip(), 't_date': self.date_entry.date().toPyDate(),
                't_matcode': self.txt_matcode.text().strip(), 't_qty': qty,
                't_note': self.txt_note.toPlainText().strip(), 't_uid': self.username,
                't_seq': self.current_record_seq}
        try:
            with self.conn.cursor() as cursor:
                if self.current_record_seq is None:
                    cols = sql.SQL(', ').join(
                        map(sql.Identifier, ['t_ctrlnum', 't_date', 't_matcode', 't_qty', 't_note', 't_uid']))
                    vals = sql.SQL(', ').join(sql.Placeholder() * 6)
                    query = sql.SQL("INSERT INTO {tbl} ({cols}) VALUES ({vals})").format(
                        tbl=sql.Identifier(TBL_INCOMING), cols=cols, vals=vals)
                    params = (
                    data['t_ctrlnum'], data['t_date'], data['t_matcode'], data['t_qty'], data['t_note'], data['t_uid'])
                    action = "saved"
                else:
                    set_clause = sql.SQL(', ').join([sql.SQL("{} = %s").format(sql.Identifier(k)) for k in
                                                     ['t_ctrlnum', 't_date', 't_matcode', 't_qty', 't_note']])
                    query = sql.SQL("UPDATE {tbl} SET {set_clause} WHERE {id_col} = %s").format(
                        tbl=sql.Identifier(TBL_INCOMING), set_clause=set_clause, id_col=sql.Identifier(COL_SEQ))
                    params = (
                    data['t_ctrlnum'], data['t_date'], data['t_matcode'], data['t_qty'], data['t_note'], data['t_seq'])
                    action = "updated"
                cursor.execute(query, params)
                self.conn.commit()
    ...
    def delete_record(self):
        if self.current_record_seq is None: return
        reply = QMessageBox.question(self, "Confirm Deletion", "Are you sure you want to delete this record?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
                query = sql.SQL("UPDATE {tbl} SET {del_col} = TRUE WHERE {id_col} = %s").format(
                    tbl=sql.Identifier(TBL_INCOMING), del_col=sql.Identifier(COL_DELETED),
                ...
                self.conn.rollback(); QMessageBox.critical(self, "Database Error",
                                                           f"Failed to delete record:\n{str(e)}")

    def clear_search(self):
        self.search_input.clear(); self.load_records()
...
class FrmDashboard(QMainWindow):
    ...
    def create_side_menu(self):
        ...
        user_icon = QLabel()
        user_icon.setPixmap(fa.icon('fa5s.user-circle', color='#ecf0f1').pixmap(QSize(40, 40)))
        user_label = QLabel(f"<b>{self.username}</b><br><font color='#bdc3c7'>Administrator</font>")
        ...
        self.btn_materials.clicked.connect(lambda: self.stacked_widget.setCurrentIndex(1))
    ...
    def toggle_management_submenu(self):
        ...
        self.animation = QPropertyAnimation(self.management_submenu_container, b"maximumHeight")
