...
class LoginForm(QWidget):
    ...
    def handle_login(self):
        ...
        try:
            with self.conn.cursor() as cursor:
                cursor.execute(
                    "SELECT t_username FROM tbl_user_pass WHERE t_username = %s AND t_userpass = %s;",
            ...
            if result:
                ...
                self.dashboard_window = FrmDashboard(
                    username=result[0], connection=self.conn, login_window=self
                )
            ...
            QMessageBox.critical(
                self, "Database Error", f"An error occurred during login: {e}"
            )
            self.conn.rollback()
    ...
    def get_workstation_info(self):
        ...
        return {"wsid": hostname, "wsname": hostname}
    ...
    def log_event(self, username, status):
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute(
                    "INSERT INTO tbl_user_log02 (t_wsname, t_username, t_status) VALUES (%s, %s, %s);",
    ...
    def show_create_user_dialog(self):
        ...
        _ = (
            self.prompt_for_new_user_details()
            if ok and admin_pass == ADMIN_PASSWORD
            else (
    ...
    def prompt_for_new_user_details(self):
        username, ok1 = QInputDialog.getText(self, "Create User", "Enter new username:")
        password, ok2 = (
            QInputDialog.getText(
                self, "Create User", "Enter password:", QLineEdit.EchoMode.Password
            )
        ...
        confirm_pass, ok3 = (
            QInputDialog.getText(
                self, "Create User", "Confirm password:", QLineEdit.EchoMode.Password
            )
            if ok2 and password
            else (None, False)
        )
        _ = (
            self.create_user_in_db(username.strip(), password)
            if ok3 and password == confirm_pass
            else (
    ...
    #                 (
    #                     self.get_workstation_info()["wsid"],
    #                     self.get_workstation_info()["wsname"],
    #                     username,
    ...
    #         QMessageBox.warning(
    #             self, "Creation Failed", f"Username '{username}' already exists."
    #         )
    #         self.conn.rollback()
    #     except psycopg2.Error as e:
